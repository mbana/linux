...
# Set log file
curl --show-error -X PUT --unix-socket "${API_SOCKET}" \
  --data "{
      \"log_path\": \"${LOGFILE}\",
      \"level\": \"Trace\",
      \"show_level\": true,
...
KERNEL="$(pwd)/vmlinux-5.10.204"
KERNEL_BOOT_ARGS="console=ttyS0 reboot=k panic=1 pci=off verbose"
